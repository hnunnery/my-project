...
export default function Dashboard() {
  ...
  const fetchSleeperData = useCallback(async (forceRefresh = false) => {
    ...
    setIsRefreshingLeagues(forceRefresh)
    setSleeperError('')
              ...
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                  {(() => {
                    ...
                    if (!selectedAccount) return 'My Leagues (0)'

                    const accountData = storedLeagues[normalizeUsername(selectedAccount.username)]
                    const count = accountData?.leagues.length ?? 0
                    return `My Leagues (${count})`
                  })()}
                </h2>
                {(() => {
                  const selectedAccount = savedAccounts.find(acc => acc.isDefault)
                  ...
                  const accountData = storedLeagues[normalizeUsername(selectedAccount.username)]
                   if (!accountData) return null
                    ...
                    return (
                      ...
                      >
                        <svg
                          className={`w-5 h-5 ${isRefreshingLeagues ? 'animate-spin' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
